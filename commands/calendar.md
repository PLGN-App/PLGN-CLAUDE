...
- **A URL** — work out the topics first with `plgn-researcher` and
  `plgn-strategist`.
...
2. Choose how often to post, and **say it before the table** so it can be
   argued with.
3. Lay out 30 days.
4. Close with the **upsell-seam** skill — the **draft close**, using
   **This 30-day plan**.
...
- **Change the shape of the hook** between one slot and the next, per
  **platform-specs**. Two number-led hooks in a row read like a template.
...
- Per **_conventions** rule 6, put what the agents need into their prompts.
