...
// `_id`/`_ids`", which would let a genuinely invented tool through. The
// brief pipeline adds five more, checked the same way: `post_id` on
// brief_create/brief_get/brief_list/generate_image/generate_image_from_image,
// `brief_id` on brief_update/brief_finalize/brief_get/post_create/post_update/
// generate_image/generate_image_from_image, `knowledge_used` on brief_create/
// post_create/post_update, `slide_order` on generate_image/generate_image_from_image,
// and `planned_slides` on post_create/post_update — not speculation.
const NON_TOOL_NAMES = new Set([
  ...
  // Brand assets: `asset_ids` on generate_image_from_image, `asset_id` on the
  // asset_* tools, and the asset_create/asset_update arguments the skill shows.
...
// The research tools (1.8.0) are read-only and a free command may use them
// when plgn happens to be connected -- but only if the same file also says
// what it does without plgn, so the "no account needed" promise holds.
const RESEARCH_TOOLS = new Set(["site_read", "social_fetch", "web_search"]);
for (const c of FREE) {
  ...
  for (const t of TOOLS) {
    ...
    if (RESEARCH_TOOLS.has(t)) {
      if (!/when it is not|not connected/i.test(body)) {
        fail(`${p}: free command uses research tool \`${t}\` but never says what it does without plgn connected`);
      }
      continue;
    }
    fail(`${p}: free command must not reference MCP tool \`${t}\``);
  }
  // A free command has no brand and no plan-and-confirm step, so it may not
  // read or write the workspace -- not even through the research exemption.
  // Found in review: competitors saved `competitor` entries and audit read the
  // `channels` entry, neither with a tool it was allowed to call.
  if (/`[a-z_]+` entr(y|ies)/.test(body)) {
    fail(`${p}: free command must not read or write a knowledge entry`);
  }
  if (/\bSave each\b|\bis saved to\b/.test(body)) {
    fail(`${p}: free command must not save anything to a workspace`);
  }
}
// The free/connected contract is written down in _conventions and the README;
// once free commands may call the research tools, both must say so.
{
  const conv = exists("reference/_conventions.md") ? read("reference/_conventions.md") : "";
  if (/Call \*\*zero\*\* MCP tools/.test(conv)) {
    fail("reference/_conventions.md: the Free bullet still says free commands call zero MCP tools");
  }
  for (const t of RESEARCH_TOOLS) {
    if (!conv.includes(`\`${t}\``)) fail(`reference/_conventions.md: the Free bullet must name research tool \`${t}\``);
  }
  const readme = exists("README.md") ? read("README.md") : "";
  if (/free commands never contact/i.test(readme)) {
    fail("README.md: still says the free commands never contact the server");
  }
}
// brandkit: nothing is written before the plan's yes (so --dry-run and "no"
// write nothing). Its gathering step may read channels, never save them.
if (exists("commands/brandkit.md")) {
  const bk = read("commands/brandkit.md");
  const section = (n) => {
    const m = bk.match(new RegExp(`^## ${n}\\.[^\\n]*\\n([\\s\\S]*?)(?=^## )`, "m"));
    return m ? m[1] : "";
  };
  if (/\bsave\b/i.test(section(3))) fail("commands/brandkit.md: step 3 saves before the plan is shown and approved");
  if (!/Channels/.test(section(6))) fail("commands/brandkit.md: step 6's plan must list the Channels it will save");
  if (!/`channels`/.test(section(7))) fail("commands/brandkit.md: step 7 must save the `channels` entry");
}
...
// replies themselves. These checks keep both from drifting back.
{
  ...
  if (!conv.includes("reply-style")) {
    fail("reference/_conventions.md must point at the reply-style skill");
  }
  ...
  "refresh", "library", "brand", "knowledge", "campaign", "import-store"];
{
  ...
  for (const c of NO_YES) {
    if (!conv.includes(`\`${c}\``)) {
      fail(`reference/_conventions.md rule 4 does not list \`${c}\` among the commands that refuse --yes`);
    }
    const p = `commands/${c}.md`;
...
// 1.7.2: every key an agent returns that a command passes to a tool is now
// the tool's own snake_case argument name (the server drops unknown names
// silently), so the needles below follow.
